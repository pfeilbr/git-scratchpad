...
# The API's own alias for the signed-in user's default list. It is an
# API-defined literal rather than user data, so it is the one task list id
# that must reach the wire unescaped (`quote_id` would make it %40default).
DEFAULT_LIST = "@default"

LIST_FLAG = arg("--list", default=DEFAULT_LIST,
                help=f"task list id (default: {DEFAULT_LIST})")


def _list_url(args, suffix: str = "") -> str:
    """A task-list URL; the list id is escaped as a single segment."""
    list_id = (args.list if args.list == DEFAULT_LIST
               else quote_id(args.list))
    return f"{BASE}/lists/{list_id}{suffix}"


def _task_url(args, suffix: str = "") -> str:
    """One task's URL; both user-supplied ids are escaped."""
    return _list_url(args, f"/tasks/{quote_id(args.id)}{suffix}")
...
def cmd_list(args) -> int:
    emit_paged(args, _list_url(args, "/tasks"),
...
def cmd_add(args) -> int:
    ...
    task = Client.for_args(args).post(_list_url(args, "/tasks"),
                                      json_body=body)
...
def cmd_done(args) -> int:
    Client.for_args(args).patch(_task_url(args),
...
def cmd_rm(args) -> int:
    Client.for_args(args).delete(_task_url(args))
...
def cmd_move(args) -> int:
    ...
    Client.for_args(args).post(_task_url(args, "/move"),
...
def cmd_clear_completed(args) -> int:
    Client.for_args(args).post(_list_url(args, "/clear"))
